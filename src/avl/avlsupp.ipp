...
/******************************************************************************
   Modifications for Windows Driver Unit Test Framework (WDUTF)
   Copyright (c) 2025
******************************************************************************/
                      ...
                      IN OUT TABLE_SEARCH_RESULT SearchResult)
{
    ...
    if (SearchResult == TableEmptyTree)
    {
        ...
        RtlInsertAsRightChildAvl((PRTL_SPLAY_LINKS)&Table->BalancedRoot, (PRTL_SPLAY_LINKS)NewNode);
    ...
    {
        /* Insert it left */
        RtlInsertAsLeftChildAvl((PRTL_SPLAY_LINKS)NodeOrParent, (PRTL_SPLAY_LINKS)NewNode);
    }
    else
    {
        /* Right node */
        RtlInsertAsRightChildAvl((PRTL_SPLAY_LINKS)NodeOrParent, (PRTL_SPLAY_LINKS)NewNode);
    }
    ...
    while (TRUE)
    {
        ...
        if (RtlBalance((PRTL_BALANCED_LINKS)NodeOrParent) == RtlBalancedAvlTree)
        {
            /* It's not balanced anymore (heavy on one side) */
            RtlSetBalance((PRTL_BALANCED_LINKS)NodeOrParent, Balance);

            /* Move up */
            NewNode = (PRTL_BALANCED_LINKS)NodeOrParent;
            NodeOrParent = RtlParentAvl((PRTL_SPLAY_LINKS)NodeOrParent);
        }
        else if (RtlBalance((PRTL_BALANCED_LINKS)NodeOrParent) != Balance)
        {
            /* The parent's balance is opposite, so the tree is balanced now */
            RtlSetBalance((PRTL_BALANCED_LINKS)NodeOrParent, RtlBalancedAvlTree);
        ...
        else
        {
            ...
            RtlpRebalanceAvlTreeNode((PRTL_BALANCED_LINKS)NodeOrParent);
            break;
        }
    }
}
